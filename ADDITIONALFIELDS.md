...
NOTE: **NEVER** apply translations by translating the `Name` field, please follow the [translation guide](#translating-additional-domain-fields) instead.
...
Based on the standard [WHMCS Documentation](https://docs.whmcs.com/Additional_Domain_Fields) section `Translating a Field Name`, follow the below way to it realized the best. Explaining it for german language as example.

* Find language files listed [here](https://github.com/hexonet/whmcs-ispapi-registrar/tree/master/lang/overrides)
* Download the language file of your choice
* Copy it into folder `/lang/overrides` of your WHMCS Installation if not existing yet. Otherwise, add the contents to the end of the file. When upgrading, ensure to cleanup old translations first.

**NOTE:** All our translation key are prefixed with `hxflags` so that you can identify them in ease.
**NOTE:** Translating the `Description` or `Options` field is not possible in WHMCS. See [Feature Request](https://requests.whmcs.com/topic/finalize-translation-support-for-additional-domain-fields).
...
### Available Variables in the config files

1. `$isOTE` can be used to make configurations system entity dependent. Truthy in case you're connected to our OT&E system, otherwise falsy. Check configuration file for .NO.
2. `$type` can be used to make configurations order type dependent. Can be `register` or `transfer`. NOTE: WHMCS still provides type `register` for transfers, so thought for future use.
3. `$tld` can be used if you need to know the TLD we forward to the configuration file. This makes sense as we share some of them.
